...
let editModal;
    ...
    async function renderAdminHistory() {
        ...
        try {
            ...
            container.innerHTML = recent.map(r => `<div class="history-item"><div class="fw-bold" style="color:rgba(255,255,255,0.88);">${r.entry}</div><div class="small text-muted">${r.stamp}</div></div>`).join('')
                || '<div class="small text-muted">No log entries yet.</div>';
        } catch (error) {
            console.error(error);
            container.innerHTML = adminHistory.map(r => `<div class="history-item"><div class="fw-bold" style="color:rgba(255,255,255,0.88);">${r.entry}</div><div class="small text-muted">${r.stamp}</div></div>`).join('')
                || '<div class="small text-muted">No log entries yet.</div>';
        }
    }
